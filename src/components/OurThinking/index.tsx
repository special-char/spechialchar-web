...
function OurThinking({data}: ourThinkingType) {
  const [currentCategory, setCurrentCategory] = useState('All');

  return (
    <section>
      <div className='flex max-md:flex-col max-sm:text-2xl max-md:text-3xl m-4  text-4xl font-bold justify-start '> 
        <button type='button' onClick={() => setCurrentCategory('All')}  >
        <div className='flex justify-start mr-4 active:text-blue-600'>All</div>
        </button>
        <button type='button' onClick={() => setCurrentCategory('Strategy')} >
        <div className='flex justify-start mr-4 active:text-blue-600'>Strategy</div>
        </button>
        <button type='button' onClick={() => setCurrentCategory('Trending')}>
        <div className='flex justify-start mr-4 active:text-blue-600'>Trending</div>
        </button>
        <button type='button' onClick={() => setCurrentCategory('Development')}>
        <div className='flex justify-start mr-4 active:text-blue-600'>Development</div>
        </button>
      </div>
      <div className='grid grid-cols-2 max-md:grid-cols-1 justify-center'>
        {ourThinkingData[currentCategory === 'All' ? 'All' : currentCategory].map((x) => (
          <div
            key={x.id}
