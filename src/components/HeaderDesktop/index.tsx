      ...
      <div className="flex justify-end gap-x-[3.5rem] items-start py-2 pl-5">
        {accordionItems.data.map((item, index) => (
          <div key={index} className="flex flex-col pt-3 group">
            <div className="flex text-primary hover:text-button gap-x-1  text-xl">
              <Link
              ...
              {item?.content && (
                <ChevronDownIcon className="mt-2 transition-transform duration-300 transform group-hover:rotate-180 hover:text-button" />
              )}
            </div>
            <ul className="text-primary top-full left-0 mt-1 group-hover:mb-3 max-h-0 overflow-hidden transition-all duration-300 group-hover:max-h-screen flex flex-col gap-y-4 pt-2">
              {item?.content?.map((itemContent, subindex) => (
                <div key={subindex} className="">
                  ...
                  >
                    <span className="hover:text-button text-base flex flex-col flex-wrap">
                      {itemContent}
                    </span>
                  </Link>
                </div>
